...
const upd: ValidatedEventAPIGatewayProxyEvent<ProductType> = async (
  ...
  try {
    ...
    for (const item of items) {
      if (item.appUUID === info.appUUID && item.product === info.product && item.id !== info.id && item.published) {
        return erroValidationDataResponse({
    ...
    await productsTable.update(
      { pk: `Products#Products`, sk: `appUUID:${info.appUUID}#id:${info.id}` },
      {
        set: {
